...
const isProduction = !!(document.querySelector('#gatsby-script-loader, #gatsby-chunk-mapping'));
...
chrome.runtime.onMessage.addListener((request, sender, response) => {
  if (request.from === 'popup' && request.subject === 'DOMInfo' && sender.id) {
    ...
    const isGeneratorInfoExistsOnMeta = !!document.querySelector('meta[name=generator]');
    ...
    const gatsbyVersion = semverRegexp.exec(gatsbyGeneratorInfo)[0];
    const gatsbyInfo = {
